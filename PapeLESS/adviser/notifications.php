...
requireRole('adviser');
$userId=$_SESSION['user_id']; $userType='adviser'; $pageTitle='Notifications'; $activeNav='notifications';
$db=getDB();
$notifs=$db->prepare("SELECT * FROM notifications WHERE user_type='adviser' AND user_id=? ORDER BY created_at DESC LIMIT 50");
$notifs->execute([$userId]); $notifications=$notifs->fetchAll();
markNotificationsRead('adviser',$userId);
