...
$userId=$_SESSION['user_id']; $userType='coordinator'; $pageTitle='My Profile'; $activeNav='profile';
$db=getDB();
...
if($_SERVER['REQUEST_METHOD']==='POST'&&validateCSRF($_POST['csrf_token']??'')){
    ...
    $updates=["first_name=?,last_name=?"]; $params=[$fn,$ln];
    if($newPwd){
    ...
    if(!$error){$params[]=$userId;$db->prepare("UPDATE coordinators SET ".implode(',',$updates)." WHERE id=?")->execute($params);$_SESSION['user_name']="$fn $ln";$success='Profile updated!';}
}
$user=getCurrentUser();
include __DIR__.'/../components/header.php';
?>
<?php if($success): ?><div class="alert alert-success" style="border-radius:10px"><?=$success?></div><?php endif; ?>
  ...
  <div class="col-lg-4">
    <div class="card text-center">
      <div class="card-body pt-4">
        ...
        <p style="font-size:.8rem;color:var(--primary);font-weight:600">OJT Coordinator</p>
