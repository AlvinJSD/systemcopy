...
$userId=$_SESSION['user_id']; $userType='coordinator'; $pageTitle='Notifications'; $activeNav='notifications';
$db=getDB();
$notifs=$db->prepare("SELECT * FROM notifications WHERE user_type='coordinator' AND user_id=? ORDER BY created_at DESC LIMIT 50");
$notifs->execute([$userId]); $notifications=$notifs->fetchAll();
markNotificationsRead('coordinator',$userId);
...
?>
<div class="mb-4"><h5 class="fw-bold mb-0">Notifications</h5></div>
